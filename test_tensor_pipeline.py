#!/usr/bin/env python3
"""
Test script for tensor processing pipeline

Demonstrates the complete pipeline:
1. Create synthetic gesture data
2. Process with TensorProcessor
3. Validate output
4. Test GestureTrack integration
"""

import numpy as np
from tensor_processor import TensorProcessor, TouchPoint, validate_tensor, SEQUENCE_LENGTH
from trackpad_lib import GestureTrack
...
def test_gesture_track_integration():
    """Test GestureTrack.get_tensor_representation()"""
    print("\n\nTest 2: GestureTrack Integration")
    print("=" * 60)
    
    # Create GestureTrack
    track = GestureTrack(finger_id=0, color=(0, 255, 0))
    print(f"✓ Created GestureTrack")
    
    # Add synthetic points
    points = create_synthetic_gesture(num_points=75)
    for p in points:
        track.add_point(p.x, p.y, p.timestamp, p.timestamp_ns)
    
    print(f"✓ Added {len(track.points)} points to track")
    
    # Get tensor representation
    tensor = track.get_tensor_representation(
        sequence_length=SEQUENCE_LENGTH,
        normalization='standard'
    )
    
    if tensor is not None:
        print(f"✓ Got tensor representation: {tensor.shape}")
        ...
        print(f"❌ Failed to get tensor representation")
        return None
...
def test_edge_cases():
    ...
    print("\n3.3: Stationary gesture (all same location)")
...
def test_normalization_methods():
    ...
    print("\n4.1: Standard normalization (zero mean, unit variance)")
    ...
    print("\n4.2: MinMax normalization (0-1 range)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        test_gesture_track_integration()
        test_edge_cases()
